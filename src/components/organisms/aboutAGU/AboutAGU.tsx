...
const FALLBACK_STATS = [
  { to: 150, suffix: "+", labelKey: "AGU.stats.teachers", duration: 2 },
  { to: 20,  suffix: "",  labelKey: "AGU.stats.doctors",  duration: 2 },
  { to: 10000, suffix: "", labelKey: "AGU.stats.graduates", duration: 1 },
  { to: 65,  suffix: "%", labelKey: "AGU.stats.success",  duration: 2 },
];

const AboutAGU = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { pagesByGroup, pageDetails, fetchGroupWithDetail, groupStatus } = useCmsStore();
  ...
  const allBlocks = homePages.flatMap(p => pageDetails[p.slug]?.blocks ?? []);

  const textBlocks = allBlocks
    .filter(b => b.block_type === "text")
    .sort((a, b) => a.order - b.order);

  const numberBlocks = allBlocks
    .filter(b => b.block_type === "number")
    .sort((a, b) => a.order - b.order);

  const title = textBlocks[0]?.title || t("AGU.subTitle");
  const descriptions = textBlocks.length > 0
    ? textBlocks.map(b => b.description).filter(Boolean)
    : [t("AGU.description1"), t("AGU.description2")];

  const stats = numberBlocks.length > 0
    ? numberBlocks.map(b => ({
        to: parseFloat(b.value) || 0,
        suffix: b.value?.replace(/[\d.]/g, "") ?? "",
        label: b.title,
        duration: 2,
      }))
    : FALLBACK_STATS.map(s => ({ ...s, label: t(s.labelKey) }));
        ...
        >
          ...
          <div className="w-11/12 mx-auto py-10 sm:py-14 lg:py-20 relative z-10">
            <div className="flex items-center gap-2">
              <div className="w-6 h-1 rounded-full" style={{ backgroundColor: GOLD }} />
            ...
            <div className="flex flex-col gap-5 sm:gap-8 lg:gap-10 mt-6 sm:mt-8 lg:mt-10">
              ...
              {descriptions.map((desc, i) => (
                <p key={i} className="text-sm sm:text-base lg:text-lg text-white/70 lg:w-3/4">
                  {desc}
                </p>
              ))}
                ...
                {stats.map((s, i) => (
                  <div
                    ...
                    className={`flex flex-col gap-1.5 p-4 sm:p-6 ${
                      i % 2 === 1 ? "border-l-2 border-white/15 lg:border-l-0" : ""
                    }`}
                  >
                    <span className="font-semibold text-3xl sm:text-4xl lg:text-5xl" style={{ color: GOLD }}>
                      <CountUp
                        from={0}
                        to={s.to}
                        separator=","
                    ...
                    <p className="text-xs sm:text-sm lg:text-base text-white/60">{s.label}</p>
                  </div>
                ))}
              </div>
            </div>
