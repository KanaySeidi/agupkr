...
type ScheduleActions = {
  fetchList: (scheduleType?: ScheduleType) => Promise<void>;
...
export const useScheduleStore = create<ScheduleState & ScheduleActions>()(
  devtools(
    (set, get) => ({
      ...
      fetchList: async (scheduleType?: ScheduleType) => {
        listController?.abort();
        listController = new AbortController();
        set({ status: 'loading', error: null });
        try {
          const data = await scheduleService.list(scheduleType, listController.signal);
